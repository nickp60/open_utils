...
subdirname = os.path.join(
    home,
    "%s_reannotate_output" %str(now.strftime("%Y%m%d")))
...
genbankdf = pd.DataFrame()  ## genbank List Long
qualifiers = ["old_locus_tag","locus_tag","protein_id",
              "db_xref", "translation", "gene"]
index = 0
    ...
    for feature in record.features:
        ...
        try:
            for qualifier in qualifiers:
                try:
                    genbankdf.loc[index, qualifier] = str(
                        feature.qualifiers[qualifier]).replace(
                            "\'", "").replace("[", "",).replace("]", "")
                except KeyError:
                    genbankdf.loc[index, qualifier] = 'none'
                genbankdf.loc[index, "seq"] = str(
...
if ((genbankdf.locus_tag == 'none').all() & (genbankdf.gene == "none").all()):
    invalid_genbank = KeyError(
        "Genbank file must have at either unique locus_tag column or gene column")
...
if (genbankdf.locus_tag == 'none').any():
    print("Warning! removing %i entries lacking locus_tag.." %
...
if rename_fa:
    ...
    if len(names) > 1:
        ...
        base_name = names[0].partition(".")[0][0: -1]
        length_of_names = len(names)
        version_of_genbank = re.sub("(.*)\.(.*)$", r"\2", names[0])
        if not string.ascii_letters.find(starts_at) == -1:
            name_not_digit = KeyError(
                "not actally a key error, but at this point, fastas can " +
                "only be renamed if the sequence ends in a digit :(")
            raise name_not_digit
        if not string.ascii_letters.find(starts_at) == -1:
            version_not_digit = KeyError(
                "not actally a key error, but at this point, fastas can " +
                "only be renamed if the version ends in a digit :(")
            raise version_not_digit
        if not int(starts_at) == 1:
            version_not_digit = KeyError(
                "not actally a key error, but at this point, fastas can " +
                "only be renamed if the version starts with 1 :(")
            raise version_not_digit
        renaming_script = str(
            r"""awk '/^>/{print ">""" + base_name + """" ++i ".""" +
            version_of_genbank + r""" "; next}{print}' < """ +
            output_genfasta_handle.name + " > " +
            str(output_genfasta_handle.name.partition(".fasta")[0] +
                "_renamed.fasta"))
        print(renaming_script)
    elif len(names) == 1:
        name = names[0]
        renaming_script = str(
            r"""awk '/^>/{print ">""" + name + """"; next}{print}' < """ +
            output_genfasta_handle.name + " > " +
            str(output_genfasta_handle.name.partition(".fasta")[0] +
                "_renamed.fasta"))
        print(renaming_script)
...
if nuc_flag == True:
    noutput_handle = open(os.path.join(subdirname, gb + ".fn"), "w")
    noutput_path = os.path.join(subdirname, gb + ".fn")
...
make_db_command_target = str(
    "makeblastdb -in " + input_target_fasta +
    " -input_type fasta -dbtype prot -title " +
    input_target +
    " -parse_seqids -out " + os.path.join(home, "BLAST", bdb_name))
# for reciprocal blasting
make_db_command_query= str(
    "makeblastdb -in " + aaoutput_handle.name +
    " -input_type fasta -dbtype prot -title " + gb +
...
blast_cline_recip = NcbiblastpCommandline(
    query=input_target_fasta,
