...
def main(args):
    ...
    for key, items in gene_entries.items():
        ...
        logger.info("Running %s for MSA", "mafft")
    ...
    for msa in msas:
        ...
        # alignments = SeqIO.parse(msa, "fasta")
        # align_array = np.array([x.id for x in alignment], np.character
        # align_array = np.array(alignment, np.character)
        # print(len(align_array[:,0]))
        # print(align_array[:,0])
        # print(align_array[:,77])
        # print(len(align_array[0,:]))
    ...
    print(df_big.head())
    df_big.to_csv(os.path.join(args.output, "combined_msas.tab"), sep="\t")
