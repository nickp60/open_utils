...
Minor version changes:
 ...
 - only takes csv input.  lets be real, thats all I care about right now...
...
def run_blastn(filename):
    ...
    output_path_tab =  str(os.path.join(output, date)+"_dcmegablast_results_"+str(md5.new(str(datetime.datetime.now())).hexdigest())+".tab")
...
def run_tblastx(filename):
    # build commandline call
    output_path_tab =  str(os.path.join(output, date)+"_tblastx_results_"+str(md5.new(str(datetime.datetime.now())).hexdigest())+".tab")
...
def main():
    ...
    merged_fasta =  merge_outfiles(output_dir_files, os.path.join(output, str(date+"_results_merged.csv")))
